1. JS中6中基本的数据类型：
	...
	BigInt: es10添加，是一种数字类型的数据，它可以表示任意精度格式的整数，使用 BigInt 可以
	安全地存储和操作大整数，即使这个数已经超出了 Number 能够表示的安全整数范围。

2. JS中有两种类型的值，基本类型和引用类型，分别存放在栈和堆中

3. 堆和栈的区别：
	在数据结构中，栈中数据的存取方式是先进先出。而堆是一个优先队列，是按优先级来进行排序的，
...
6. undefined 和 undeclared 的区别？
	...
	但是我们可以使用 typeof 的安全机制来避免报错，因为对于 undeclared （或者 not defined）变量，
	typeof 会返回 'undefined'。

	在ECMAScript2015前，使用 typeof 永远不会抛出错误。

	但在加入了块级作用域的 let 和 const 之后，在其被声明之前对块中的 let 和 const 变量使用 typeof
...
7. null 和 undefined 的区别？
	...
	typeof null 会返回 'object'，这是一个历史遗留问题：
		在JS最初的实现中，JS中的值是由一个表示类型的标签和实际数据值表示的。对象的类型标签是 0，由于 null 代表的是空指针
		（大多数平台下值为 0x00），因此 null 的类型标签是 0， typeof null 也因此返回 'object'。
...
10. JS原型，原型链？有什么特点？
	...
	于是就这样一直找下去，也就是原型链的概念。原型链的尽头一般来说都是 Object.prototype 所以这就是我们新建的对象为什么能够使用
	toString() 等方法的原因。
	在 js 中，我们是使用构造函数来新建一个对象的，每一个构造函数的内部都有一个 prototype 属性值，这个属性值是一个对象，
	这个对象包含了可以由该构造函数的所有实例共享的属性和方法。
	当我们使用构造函数新建一个对象后，在这个对象的内部将包含一个指针，这个指针执行构造函数的 prototype 属性对应的值，在 ES5
	中这个指针被称为对象的原型。
	一般来说我们是不应该能够获取这个值的，但是现在浏览器中都实现了 __proto__ 属性来让我们访问这个属性，但是我们最好不要使用
	这个属性，因为它不是规范中规定的。
...
13. js 中整数的安全范围是多少？
	安全整数指的是，在这个范围内的整数转化为二进制存储的时候不会出现进度丢失，能够被“安全”呈现的最大整数是2^53 - 1，即
	9007199254740991，在ES6中被定义为 Number.MAX_SAFE_INTEGER。最小整数是 -9007199254740991，在ES6中被定义为
	Number.MIN_SAFE_INTEGER。
	如果某次计算的结果得到了一个超过JS数值范围的值，那么这个值会被自动转换为特殊的 Infinity 值。如果某次计算返回了正或负
	Infinity 值，那么该值将无法参与下一次的计算。
	判断一个值是不是有穷的，可以使用 isFinite 函数来判断。

14. typeof NaN 的结果是什么？
	NaN 意指不是一个“数字”(not a number)，NaN 是一个“警戒值”(sentinel value， 有特殊用途的常规值)，用于指出数字类型中的
	错误情况，即“执行数学运算没有成功，这是失败后返回的结果”。
	typeof NaN; // "number"
...
16. Array 构造函数只有一个参数值时的表现？
	如果传递给Array构造函数的唯一参数是介于0和232-1（含）之间的整数，这将返回一个新的JavaScript数组，其长度属性设置为该数字
	（注意：这意味着实现了一个arrayLength空槽的数组，空槽不具有实际未定义值 undefined）。如果参数是任何其他数字，则会抛出一个RangeError异常。
...
37. Javascript 的作用域链？
	...
	自由变量：当前作用域没有定义的变量。
	作用域链：如果变量在当前作用域没有找到，就去父级作用域寻找，如果父级作用域也没有，就一层一层往上寻找，
	直到找到全局作用域还是没找到，就宣布放弃。这种一层一层的关系，就是作用域链。
	...
	作用域与执行上下文不是相同的概念：
		...
		执行阶段：
			...
			执行函数代码
			垃圾回收
		JS 解释阶段便会确定作用域规则，因此作用域在函数定义时就已经确定了，而不是在函数调用时确定，但是执行上下文是函数执行
		之前创建的。执行上下文最明显的就是 this 的指向是执行时确定的。
	作用域访问的变量是编写代码的结构确定的。
	作用域和执行上下文之间最大的区别是：
		执行上下文在运行时确定，随时可以改变；
		作用域在定义时就确定，并且不会改变。
	一个作用域下可能包含若干个上下文环境。有可能从来没有过上下文环境（函数从来没有被调用过）；有可能有过，现在函数被调用完毕后，
	上下文环境被销毁了；有可能同时存在一个或多个（闭包）。同一个作用域下，不同的调用会产生不同的执行上下文环境，继而产生不同的
	变量的值。
...
39. eval 是做什么的？
	...
	eval 的作用域：
		...
		说明作用域在它所有的范围内有效
	安全问题：
		JS中的 eval() 不安全，可能会被利用做XSS攻击（跨站脚本攻击 Cross Site Scripting)，eval 也存在一个安全问题，
		因为它可以执行传给它的任何字符串，所以永远不要传入来历不明或者不受信任源的参数字符串。
	注意事项：
...
40. 什么是 DOM 和 BOM？
	...
	BOM是浏览器对象模型，提供了独立于内容而与浏览器窗口交互的对象。描述了与浏览器进行交互的方法和接口，可以对浏览器窗口进行访问
	和操作

	DOM是文档对象模型，是针对XML的基于树的API。描述了处理网页内容的方法和接口，是HTML和XML的API，DOM把整个页面规划成由节点
	层级构成的文档。
	BOM的最根本对象是window
...
46. 什么是闭包，为什么要用它？
	...
	作用：
		匿名自执行函数、更优雅，更简洁的表达出代码、在某些方面提升代码执行效率、实现封装、实现面向对象中的对象、缓存
		可读取其他函数内部的变量
		让这些变量的值始终保存在内存中（闭包使得JS的垃圾回收机制GC不会收回外部函数所占用的资源，因为外部函数的内部函数的执行需要
		依赖外部函数中的变量）
